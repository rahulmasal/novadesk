            ...
            <div className={`w-full max-w-md mx-4 rounded-2xl p-6 ${isLightTheme ? "bg-white" : "bg-neutral-900"}`} onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`flex items-center gap-1 ${systemStatus?.api.status === "operational" ? "text-emerald-400" : "text-red-400"}`}>
                    <Activity className="w-4 h-4" /> {systemStatus?.api.status === "operational" ? "Operational" : "Error"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className={isLightTheme ? "text-gray-900" : "text-white"}>Database</span>
                  <span className={`flex items-center gap-1 ${systemStatus?.database.status === "operational" ? "text-emerald-400" : "text-red-400"}`}>
                    <Activity className="w-4 h-4" /> {systemStatus?.database.status === "operational" ? "Operational" : "Error"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className={isLightTheme ? "text-gray-900" : "text-white"}>WebSocket</span>
                  <span className={`flex items-center gap-1 ${isLightTheme ? "text-emerald-600" : "text-emerald-400"}`}><Activity className="w-4 h-4" /> Unknown</span>
                </div>
              </div>
            </div>
          </div>
